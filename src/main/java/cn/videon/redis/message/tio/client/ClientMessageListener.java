...
public class ClientMessageListener implements ClientAioListener {
    ...
    public void onAfterConnected(ChannelContext channelContext, boolean isConnected, boolean isReconnect) throws Exception {
        ...
        if (isConnected) {
            ...
            redisTemplate.convertAndSend("point_connect", jsonObject.toJSONString());
            recordService.findAll()
                    .forEach(record -> {
                        log.debug("send message is {}",record);
                        if (Tio.send(channelContext, TioUtil.toPacket(record.getJsonObject()))) {
                            recordService.del(record.getId());
                        }
                    });
        }
    }
